...
{
   // Create graph labeling and create node-coordinate maps over coarse graph
   ...
   vector<Coord> &coarse_grid_cells)
{
   ...
   for (Coord coord : grid_cells)
   {
      ...
      const auto coarse_coord_visited = find(coarse_grid_cells.begin(), 
         coarse_grid_cells.end(), coarse_coord);
                              ...
                              int coarse_ne)
{
   ...
   Table coarse_graph_fixed = RemoveSelfConnections(coarse_graph);
